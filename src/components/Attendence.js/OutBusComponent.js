...
const AttendenceComponent = ({ navigation }) => {
    ...
    const changeStation = async (data) => {
        ...
        isStudentList.map((value)=> {
            if(value.data.station.getOutBusFromHouse && scheduleInfo.process.destination === 1) {
                qty+=1;
            } else if(value.data.station.getOutBusFromSchool && scheduleInfo.process.destination === 2) {
                qty+=1;
            }
        })
    ...
    const goStart = async () => {
        ...
        if(isSchedule.data.process.destination === 1) {
            Alert.alert(
                "Thành công!",
                "Bắt đầu xuống xe từ Nhà đến Trường",
                [
                    { text: "OK"  }
                ]
            );
            ...
            Alert.alert(
                "Thành công!",
                "Bắt đầu xuống xe từ Trường về Nhà",
                [
                    { text: "OK"  }
                ]
            );
    ...
    const goEnd = async () => {
        ...
        if(isSchedule.data.process.destination === 1) {
            Alert.alert(
                "Thành công!",
                "Kết thúc xuống xe từ trường Nhà đến Trường",
                [
                    { text: "OK"  }
                ]
            );
            ...
            Alert.alert(
                "Thành công!",
                "Kết thúc xuống xe từ trường Trường về Nhà",
                [
                    { text: "OK"  }
                ]
            );
    ...
    const getAlertCheck = () => {
        ...
        Alert.alert(
            "Kiểm tra",
            `Có ${infoStudentList.length - qty} học sinh chưa điểm danh xuống xe`,
            [
                {
                    text: "Trở lại",
                    style: "cancel"
    ...
    const Item = ({ item }) => {
        if(scheduleInfo.process.destination === 1) {
            return (
                <View>
                    {
                        item.data.station.getOutBusFromHouse
                    ?
                    (
                    <TouchableOpacity 
            ...
            return (
                <View>
                    {
                        item.data.station.getOutBusFromSchool
                    ?
                    (
                    <TouchableOpacity 
                        onPress={() => navigation.navigate('ViewProfileStudent', {
                ...
                ?
                    <View style={styles.container}>
                        ...
                        <View style={styles.info_station_space}>
                            ...
                            {
                                scheduleInfo.process.destination !== 0 && scheduleInfo.status.getOutBus == true
                                ?
                                <View style={{ flex: 1 , 
                                    marginVertical: 10, 
                                    backgroundColor: '#fff', 
                                    ...
                                    <View style={{ flex: 1 }}>
                                        <View style={{ flex: 1, paddingVertical: 10 }}>
                                            ...
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 5 }}>
                                                ...
                                                <Text style={{ color: '#2E86C1', marginHorizontal: 5, fontSize: 13 }}>Xuống xe: {``}
                                                    {
                                            ...
                                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                                <MaterialCommunityIcons name="exit-to-app" size={24} color="#2E86C1" />
                                                <Text style={{ color: '#2E86C1', marginHorizontal: 5, fontSize: 13 }}>
                                                    Xuống xe tại bến: {``}
                                                    { qtyAttendanced }
                                                    /
                                                    {studentListAtStation.length}
